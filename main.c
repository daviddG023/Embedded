...
#include "drivers/rfidDriver/rc522.h"
...
int main() {
    ...
    while (true) {
         uint8_t version = rc522_read(0x37);  // Read VersionReg

        // Format the version into a string
        snprintf(rfid_message, sizeof(rfid_message), "RC522 Version: 0x%02X", version);

        // Print the string to the LCD
        lcd_print(rfid_message);

        sleep_ms(2000);  // Wait for 1 second before the next read // Wait for 1 second
        lcd_send_byte(LCD_CLEAR, true); // Clear the LCD for further use
        sleep_ms(2000); // Pause to show the message
